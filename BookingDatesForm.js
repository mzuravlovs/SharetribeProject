...
const calculateLineItems = (
  ...
  // Get booking dates either from root or nested structure
  const { startDate, endDate } = formValues.values?.bookingDates || formValues.values || {};
  
  // Get seats and helmet fee from form values
  const seats = formValues.values?.seats;
  const hasHelmetFee = formValues.values?.helmetFee?.length > 0;

  // Calculate seat count if seats are enabled
